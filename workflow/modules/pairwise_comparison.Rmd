...
```{r load-data}
treatment_peaks <- macs2_path %>%
  file.path(glue("{targets}_treatment_peaks.rds")) %>%
  unique() %>% 
  sapply(read_rds) %>% 
...
```{r plot-union, results = 'hide', eval = length(unique(targets)) > 1, eval = length(unique(targets)) > 1}
fig_name <- targets %>% 
  paste(collapse = "_") %>% 
  paste0("_common_peaks.", fig_type)
lapply(
  union_peaks,
  function(x) as.character(subsetByOverlaps(all_union, x))
)%>% 
  venn.diagram(
...
`r if (length(unique(targets)) > 1) glue("![*Union of peaks found in {targets[[1]]} and {targets[[2]]}, and which of the two targets each peak is found in. Peaks are based on the union of both sets of union peaks, which are themselves independent of treatment group.*](assets/{basename(fig_path)}/{fig_name})")`

## Treatment Specific Peaks

```{r treatment-peaks-upset, fig.height = 6, fig.cap = glue("*Macs2 peaks detected for ", glue_collapse(unique(targets), sep = " and " ), " in the treatment groups ", glue_collapse(unique(unlist(pairs)), sep = ", ", last = " or "), ".*")}
...
samples %>% 
  ...
  lapply(
    function(x) {
      gr <- subsetByOverlaps(all_union, treatment_peaks[[x$target]][[x$treat]])
      as.character(gr)
    }
  ) %>% 
  setNames(
  ...
  upset(
    ...
    order.by = "freq",
    set_size.show = TRUE, 
    set_size.scale_max = treatment_peaks %>% 
...
fl <- seq_along(pairs) %>% 
  lapply(
    function(x) {
      file.path(
        macs2_path[[x]], glue("{x}_{pairs[[x]]}_merged_treat_pileup.bw")
      ) %>% 
