...
if (nrow(dplyr::filter(df, if_all(samples$label)))) {
  ql <- c(
    ql,
    list(
      upset_query(
        intersect = samples$label, only_components = "intersections_matrix", 
  ...
  treat_levels %>% 
    lapply(
      function(x) {
        new_df <- dplyr::filter(
          df, 
          if_all(dplyr::filter(samples, treat == x)$label), 
          !if_any(dplyr::filter(samples, treat != x)$label)
        )
        out <- NULL
        if (nrow(new_df) > 0) {
          out <- upset_query(
            intersect = dplyr::filter(samples, treat == x)$label,
            only_components = "intersections_matrix",
            color = colours$treat[x], fill = colours$treat[x]
          )
          out
        }
...
```{r plot-peak-width, fig.cap = "*Range of peak widths across all samples. The median value for all peaks is shown as the dashed horizontal line*"}
individual_peaks %>% 
  width() %>% 
  as.list() %>% 
  lapply(list) %>% 
