...
```{r read-samples}
samples <-read_tsv(file.path(macs2_path, target, "qc_samples.tsv"))
...
macs2_logs <- file.path(
    macs2_path, samples$sample, glue("{samples$sample}_callpeak.log")
  ) %>%
  importNgsLogs() %>%
  dplyr::select(
...
file.path(macs2_path, target, "cross_correlations.tsv") %>% 
  read_tsv() %>% 
...
treatment_peaks <- treat_levels %>%
  sapply(
    function(x) {
      gr <- file.path(macs2_path, target, glue("{x}_merged_peaks.narrowPeak")) %>%
        importPeaks(seqinfo = sq, blacklist = blacklist) %>% 
        unlist()
...
bwfl <- list2(
  "{target}" := macs2_path %>%
  file.path(target, glue("{treat_levels}_merged_treat_pileup.bw")) %>% 
  BigWigFileList() %>% 
  setNames(treat_levels)
)
## The features track
...
all_out <- list(
  union_peaks_bed = file.path(macs2_path, target, "union_peaks.bed"),
  treatment_peaks_rds = file.path(macs2_path, target, "treatment_peaks.rds"),
  renv = file.path(
  ...
  c(
    sapply(
      ...
      function(x) {
        file.path(macs2_path, target, glue("{x}_treatment_peaks.bed"))
      },
      simplify = FALSE
    ) %>% 
      setNames(
        glue("{names(.)}_treatment_peaks_bed")
      )
  )
...
names(treatment_peaks) %>% 
  lapply(
    function(x) {
      id <- glue("{x}_treatment_peaks_bed")
