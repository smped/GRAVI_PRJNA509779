---
title: 'AR: E2DHT Vs. E2 Compared To H3K27ac: E2DHT Vs. E2'
...
# threads <- 6
# pairs <- list(AR = c("E2", "E2DHT"), ER = c("E2", "E2DHT"))
```
...
combs <- paste(
  ...
  pull("combn")
group_colours <- c(
  '#e6194B', '#3cb44b', '#fac100', '#4363d8', '#f58231', '#42d4f4', '#f032e6', 
...
The insistence on the more stringent initial inclusion criteria still protects against false discoveries from the prespective of window identification, but this approach instead allows *improved classification* across both comparisons.
...
df %>% 
  ...
  # geom_smooth(
  #   aes(!!sym(glue("{c1}_logFC")), !!sym(glue("{c2}_logFC"))), 
  #   data = . %>% dplyr::filter(!grepl("Undetected", status)),
  ...
  geom_label_repel(
    ...
    data = . %>% 
      ...
      dplyr::filter(d == max(d), .by = status) %>% 
      mutate(
    ...
    size = 4, fill = rgb(0.9, 0.9, 0.9, 0.2),
    show.legend = FALSE
  ) +
  annotate(
    "text", 
  ...
  theme(
    ...
    legend.position = c(0.52 * (1 - 0.2), -0.1), 
...
df %>% 
  ...
  geom_label_repel(
    ...
    force_pull = 1/10, force = 10,
    min.segment.length = 0.05,
    size = 3.5, fill = rgb(0.9, 0.9, 0.9, 0.2),
    show.legend = FALSE
  ) +
  geom_text(
    aes(x, y, label = label),
...
all_windows %>% 
  ...
  mutate(status = fct_lump_min(status, 10)) %>% 
  plotSplitDonut(
    outer = "region", 
    inner = "status",
...
grl_to_plot <- all_windows %>% 
  ...
  .[vapply(., length, integer(1)) > 10] %>% # Only show groups with > 10 ranges
  lapply(
    mutate, 
    centre = case_when(
...
sig_profiles <- mclapply(
  ...
  function(x) getProfileData(
    bwfl_fe, x, upstream = profile_width / 2, bins = n_bins, log = FALSE
  ),
...
msigdb <- msigdbr(species = params$enrichment$species) %>% 
  dplyr::filter(
