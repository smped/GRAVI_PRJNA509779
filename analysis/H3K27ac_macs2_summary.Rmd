...
all_out <- list(
  consensus_peaks_bed = file.path(macs2_path, target, "consensus_peaks.bed"),
  oracle_peaks_rds = file.path(macs2_path, target, "oracle_peaks.rds"),
  renv = file.path(
  ...
  c(
    sapply(
      names(oracle_peaks), 
      function(x) {
        file.path(macs2_path, target, glue("{x}_oracle_peaks.bed"))
      },
      simplify = FALSE
    ) %>% 
      setNames(
