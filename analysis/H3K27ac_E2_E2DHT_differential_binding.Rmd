---
title: 'H3K27ac Differential Binding: E2DHT Vs. E2'
...
As RNA-seq data `r ifelse(has_rnaseq, "was", "was not")` provided, the genes considered for enrichment analysis are the `r comma(length(unique(genes_gr$gene_id)))` `r ifelse(has_rnaseq, "genes considered as detected in the RNA-Seq data", "annotated genes")`.
...
pwf_diff <- tibble(gene_id  = unique(genes_gr$gene_id)) %>% 
...
pwf_up <- tibble(gene_id  = unique(genes_gr$gene_id)) %>% 
  left_join(mapped_ids, by = "gene_id") %>% 
  dplyr::filter(!is.na(n_peaks)) %>% 
  mutate(diff = up) %>% 
  with(
...
pwf_down <- tibble(gene_id  = unique(genes_gr$gene_id)) %>% 
  left_join(mapped_ids, by = "gene_id") %>% 
  dplyr::filter(!is.na(n_peaks)) %>% 
  mutate(diff = down) %>% 
  with(
